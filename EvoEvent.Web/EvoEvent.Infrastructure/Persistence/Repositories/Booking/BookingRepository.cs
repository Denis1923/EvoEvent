...
namespace EvoEvent.Infrastructure.Persistence.Repositories
{
	public class BookingRepository : IBookingRepository
	{
		...
		public async Task<List<Booking>> GetBookingUserAsync(Guid userId, CancellationToken token = default)
			=> await _context.Bookings.Where(b => b.UserId == userId).ToListAsync(token);
