...
### User
 ...
 - GET /events - возвращает все события (200 OK) 
 - GET /events/{id} - возвращает данные по id событию (200 OK) 
...
## Получение JWT-токена
 ...
 - Вызвать POST /auth/login, передава параметры: login и Password
 - Скопировать из ответа строковое занчение параметра Token

## Настройки JWT-конфигурации

 - Key - секрет (должен храниться Нужно хранить ключ в переменных окружения, секрет-менеджерах (Azure Key Vault, AWS Secrets Manager, HashiCorp Vault), User Secrets — для локальной разработки.)
 - Issuer - Кто выдал токен
 - Audience - Для кого предназначен токен
 - Expires - Время жизни токена
